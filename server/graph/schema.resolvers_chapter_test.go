...
func TestChapter(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expect: func() string {
				return `{"data":{"chapter":{"updatedAt":"` + testC.UpdatedAt.Format(time.RFC3339) + `"}}}`
			},
		},
		{
			des: "resolves manga",
			...
			query: `{"query":"{chapter(mangaID: 1, chapterNum: 1) {id manga {id}}}"}`,
			expect: func() string {
				return `{"data":{"chapter":{"id":"1__1","manga":{"id":1}}}}`
			},
		},
	}
...
func TestChapterList(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expect: func() string {
				return `{"data":{"chapterList":[{"updatedAt":"` + testC.UpdatedAt.Format(time.RFC3339) + `"}]}}`
			},
		},
		{
			des: "resolves manga",
